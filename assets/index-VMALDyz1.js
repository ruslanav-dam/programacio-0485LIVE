...
package com.google.maps.api;`}),e.jsx(f,{title:"Exemple Complet"}),e.jsxs(ce,{children:[e.jsx(U,{title:"Estructura del projecte",variant:"blue",children:e.jsx(p,{language:"bash",code:`projecte/
  src/
    utils/
      Matematiques.java
    app/
...
Resultat: 0.0`}),e.jsx(f,{title:"Per què és millor que tot al main?"}),e.jsxs(ce,{children:[e.jsx(U,{title:"Tot al main (monolític)",variant:"bad",children:e.jsx(p,{code:`public static void main(String[] args) {
    double a = 20, b = 5;
...
System.out.println("Valor absolut: " + absolut); // 42`}),e.jsx(f,{title:"Ternari aniuat (existeix, però no el recomanem)"}),e.jsx(d,{variant:"warning",title:"Evita els ternaris aniuats",children:e.jsxs("p",{children:["Tècnicament és possible encadenar ternaris, però el resultat és un codi molt difícil de llegir i mantenir. ",e.jsx("strong",{children:"No ho recomanem."})]})}),e.jsxs(ce,{children:[e.jsx(U,{title:"Ternari aniuat (difícil de llegir)",variant:"bad",children:e.jsx(p,{code:`// NO recomanat!
...
var x; // NO COMPILA (quin tipus es?)`})]})]}),e.jsx(V,{headers:["Situació","Es pot usar var?"],rows:[{cells:["Variable local amb assignació","Sí"]},{cells:["Variable en bucle for/for-each","Sí"]},{cells:["Atribut de classe","No"]},{cells:["Paràmetre de mètode","No"]},{cells:["Tipus de retorn","No"]},{cells:["Sense valor inicial","No"]}]}),e.jsx(f,{title:"Quan usar var i quan no"}),e.jsxs(ce,{children:[e.jsxs(U,{title:"Bon ús de var",variant:"good",children:[e.jsx("p",{children:"Quan el tipus és obvi pel context:"}),e.jsx(p,{code:`// El tipus es evident
...
}`,iA=`// Ruta RELATIVA: a partir d'on s'executa el programa
...
// No te interficie grafica. No es pot compartir.`,cA=`// AMB SPRING BOOT: aplicacio WEB
...
// Te interficie grafica (HTML/CSS). Es pot compartir.`,uA=`// Que passa quan escrius "localhost:8080/hola" al navegador?
...
// 4. El navegador REP la RESPOSTA (Response) i la MOSTRA`,dA=`// MVC a CONSOLA (el que ja sabeu):
...
System.out.println("2. Afegir animal");`,pA=`// MVC a SPRING BOOT (el que aprendreu):
...
└── pom.xml                           <- Dependencies (com un "package.json")`,hA=`// Que necessites instal·lat:
...
//   - Thymeleaf
//
// Clica "GENERATE" -> descarrega un .zip`,fA=`// AQUEST FITXER JA EXISTEIX - NO EL TOQUEU
...
}`,vA=`@Controller                    // 1. "Soc un controller, rep peticions"
public class HolaController {
...
</html>`,SA=`<!-- Thymeleaf: com passar dades de Java a HTML -->
...
<!-- \${nom} busca el valor de "nom" que has posat al Controller -->`,EA=`// ARRANCAR EL PROJECTE:
...
//   http://localhost:8080
//   http://localhost:8080/hola`,CA=`holamundo/
...
└── pom.xml                              <- JA EXISTEIX (no tocar)`,AA=`// EXERCICI: Canvia el nom dinàmicament
...
// Fixa't: el ?nom=Anna de la URL arriba al parametre "nom" del metode!`,wA=`// @RequestParam: com rebre dades de la URL
...
//
// GENERATE -> descomprimir -> obrir amb IntelliJ / VS Code`,MA=`<!-- pom.xml (extracte): la "llista de la compra" del projecte -->
...
</dependencies>`,TA=`# src/main/resources/application.properties
...
spring.thymeleaf.cache=false`,PA=`// src/main/java/com/exemple/endevinaanimal/model/Animal.java
...
// accessible al navegador a: http://localhost:8080/images/gos.jpg`,qA=`<!-- AQUESTA es la linia que afegirem a cada HTML que creem.
...
<!-- Que es? Un <link> que carrega el CSS de Bootstrap des d'internet.
     ...
     CSS de Bootstrap ja estan disponibles a la pagina. -->`,kA=`<!-- ASI quedara el <head> de joc.html (al Pas 6): -->
...
</html>`,OA=`// src/main/java/com/exemple/endevinaanimal/controller/JocController.java
...
}`,DA=`<!-- src/main/resources/templates/joc.html -->
...
</html>`,IA=`<!-- src/main/resources/templates/encertat.html -->
...
└── pom.xml`,FA=`// Que passa quan l'usuari escriu "Gos" i clica "Endevinar"?
...
// CONTROLLER -> JocController.java (coordina)`,BA=`// REPTE per al proxim dia (no cal fer-ho ara)
//
...
SELECT * FROM contactes;`,HA=`// Fluxe NOU (la fusio):
//
...
List<Contacte> tots = repo.findAll();   // <-- aixo executa SELECT * FROM contactes`,VA=`   [ NAVEGADOR ]
        |  ^
   HTTP |  | HTML
        v  |
...
// 30 linies per fer un SELECT *. Imagina'm 200 endpoints aixi.`,$A=`public List<Contacte> trobarTots() {
...
// 1 linia en lloc de 30. Mateixa BBDD, mateix resultat.`,QA=`   ┌──────────────────────┐                  ┌──────────────────────┐
...
//   5) Si dema vols cau, has de ficar-la a tot arreu.`,YA=`   ABANS (sense DAO):
   --------------------
   ...
   }`,KA=`// === COM SERIA UN DAO "fet a ma" amb JDBC ===
...
//   }
//
// El Controller ja NO veu SQL. Es un DAO "fet a ma" amb JDBC.`,XA=`   ┌─────────────────┐                       ┌─────────────────┐
   │   class         │                       │   TABLE         │
   ...
   └───────│───────────────────────────────────────────────┘
        ...
        ┌─────┐
        │ DB  │   MySQL al disc
        └─────┘`,ew=`@Entity                              // <-- ORM: aquesta classe és una taula
...
// la xarxa de la VM perque exposi el port 3306. Veure "Alternativa B".)`,rw=`// === Descàrrega de MySQL Installer per Windows ===
//
...
// Linux:  sudo apt install mysql-server mysql-workbench`,nw=`// === Configuració de l'instal·lador (Windows) ===
//
...
//
//   Apply Configuration  ->  Execute  ->  Finish.`,iw=`// === Comprovar que tot funciona ===
//
...
//
// Si Workbench obre sense errors -> MySQL local llest. ✅`,sw=`-- A MySQL Workbench, obre una nova SQL tab i executa:
...
-- Has de veure les 3 files. Si va be, la BBDD esta llesta. ✅`,lw=`// === Crear el projecte Spring Boot a start.spring.io ===
//
...
//
// Aquesta carpeta es la que obrirem amb l'IDE al pas seguent.`,ow=`// === Obrir i executar el projecte amb IntelliJ IDEA ===
//
...
//     Si surt error de connexio a MySQL: revisa la pass al properties (pas seguent).`,cw=`// === Obrir i executar el projecte amb Visual Studio Code ===
//
...
//        Started AgendaApplication in 3.245 seconds
//
//     Si surt: ✅ tot el projecte funciona.`,uw=`# src/main/resources/application.properties
#
...
spring.thymeleaf.cache=false`,dw=`// === ALTERNATIVA B: Connectar a la VM que ja teniu ===
//
...
// fan que tot s'enllaçi.`,hw=`// src/main/java/com/exemple/agenda/model/Contacte.java
...
}`,xw=`   Java (Contacte)              MySQL (taula contactes)
//   ----------------             -----------------------
//   @Entity              <-->    una FILA
...
//   4) Recupera l'id que MySQL ha generat i el posa al camp id de l'objecte.`,gw=`// src/main/java/com/exemple/agenda/repository/ContacteRepository.java
...
// Spring genera la implementació a runtime gràcies a JpaRepository<>.`,jw=`// src/main/java/com/exemple/agenda/controller/ContacteController.java
...
}`,vw=`// 1. Spring llegeix application.properties.
//
...
//               from contactes c1_0
//
// És la query que abans escrivies tu a Workbench, ara generada per Hibernate.`,bw=`// src/main/java/com/exemple/agenda/DemoCRUD.java
...
}`,yw=`# === SORTIDA ESPERADA A LA CONSOLA ===
...
===============================`,Sw=`-- === Verificacio visual a MySQL Workbench ===
...
-- visible des de Workbench. Magia? No: JPA + Hibernate.`,Ew=`// === Tu escrius Java; Hibernate executa SQL contra MySQL ===
...
└── src/main/
    ...
    └── resources/
        ...
        └── templates/
            ├── llista.html                         (PAS 5)
            └── formulari.html                      (PAS 6)`,ww=`# src/main/resources/application.properties
...
spring.thymeleaf.cache=false`,Lw=`// src/main/java/com/exemple/agenda/model/Contacte.java
...
}`,Rw=`// src/main/java/com/exemple/agenda/repository/ContacteRepository.java
...
public interface ContacteRepository extends JpaRepository<Contacte, Long> {
    ...
    List<Contacte> findByNomContainingIgnoreCase(String fragment);
}`,Mw=`// src/main/java/com/exemple/agenda/controller/ContacteController.java
...
}`,Tw=`<!-- src/main/resources/templates/llista.html -->
...
</html>`,Pw=`<!-- src/main/resources/templates/formulari.html -->
...
// 7. Edita, esborra, cerca... el CRUD complet funciona.`,qw=`// === IDEES PER ESTENDRE L'EXEMPLE ===
//
...
}`,label:"Controlador de Reserves",showLineNumbers:!0}),e.jsx(f,{title:"On trobaras MVC?"}),e.jsx(d,{variant:"info",title:"MVC es a tot arreu",children:e.jsx("p",{children:"Practicament tots els frameworks moderns utilitzen MVC o una variant. Aqui teniu exemples reals:"})}),e.jsx(V,{headers:["Plataforma","Tecnologia","Com aplica MVC"],rows:[{cells:["Java Web","Spring MVC (Spring Boot)","Controllers reben peticions HTTP, Models son les entitats, Vistes son plantilles HTML"]},{cells:["Android","Activities + Layouts + ViewModel","XML layouts (Vista), Activities/ViewModel (Controller), classes de dades (Model)"]},{cells:["Web Frontend","React, Angular, Vue","Components (Vista), estat/stores (Model), gestors d'events (Controller)"]},{cells:["Desktop","JavaFX (FXML + Controller)","Fitxers FXML (Vista), classes Controller, classes de dades (Model)"]},{cells:["Jocs","Unity (C#)","Usa una variant de MVC per separar logica de joc, renderitzat i input"]}]}),e.jsx(f,{title:"Connexio amb el que heu apres"}),e.jsxs(d,{variant:"explanation",title:"Ja teniu les peces!",children:[e.jsxs("p",{children:["Tot el que heu apres fins ara (classes, objectes, encapsulacio, packages, modularitat) son els ",e.jsx("strong",{children:"blocs de construccio"})," de MVC:"]}),e.jsxs("p",{style:{marginTop:"8px"},children:["El ",e.jsx("strong",{children:"Model"})," son les vostres classes amb encapsulacio (atributs privats, getters, setters). Els ",e.jsx("strong",{children:"packages"})," son la separacio en carpetes. La ",e.jsx("strong",{children:"modularitat"})," es el principi que cada classe fa una sola cosa. MVC simplement ",e.jsx("strong",{children:"formalitza el patro"}),": us diu on posar cada peca."]})]}),e.jsxs(d,{variant:"success",title:"La idea clau",children:[e.jsxs("p",{children:["MVC es una ",e.jsx("strong",{children:"manera de pensar"})," sobre l'organitzacio del codi, no una eina nova que cal aprendre. Ja coneixeu les peces: classes, objectes, encapsulacio, packages. MVC simplement us diu ",e.jsx("strong",{children:"on posar-les"}),": les dades al Model, la presentacio a la Vista i la coordinacio al Controller."]}),e.jsx("p",{style:{marginTop:"8px"},children:"Quan comenceu a treballar amb frameworks com Spring Boot o JavaFX, veureu que tot segueix aquest patro. Si enteneu MVC, enteneu l'arquitectura de qualsevol aplicacio moderna."})]})]})}const Qw="_quizContainer_x2w5i_1",Jw="_progressBar_x2w5i_11",Ww="_progressDot_x2w5i_23",Yw="_done_x2w5i_39",Kw="_correct_x2w5i_47",Xw="_wrong_x2w5i_55",Zw="_active_x2w5i_63",eL="_questionCard_x2w5i_73",tL="_questionNumber_x2w5i_111",aL="_questionType_x2w5i_129",rL="_questionText_x2w5i_155",nL="_codeContext_x2w5i_173",iL="_codeGap_x2w5i_205",sL="_options_x2w5i_225",lL="_option_x2w5i_225",oL="_disabled_x2w5i_271",cL="_selected_x2w5i_283",uL="_optionLetter_x2w5i_323",dL="_optionCode_x2w5i_383",pL="_optionText_x2w5i_397",mL="_feedback_x2w5i_407",hL="_fadeIn_x2w5i_1",xL="_feedbackTitle_x2w5i_449",gL="_navRow_x2w5i_461",fL="_nextBtn_x2w5i_473",jL="_results_x2w5i_509",vL="_scoreCircle_x2w5i_519",bL="_great_x2w5i_545",yL="_good_x2w5i_557",SL="_low_x2w5i_569",EL="_scoreNumber_x2w5i_581",CL="_scoreLabel_x2w5i_591",AL="_resultsTitle_x2w5i_607",wL="_resultsSubtitle_x2w5i_621",LL="_resultsSummary_x2w5i_633",RL="_statBox_x2w5i_647",ML="_statNumber_x2w5i_655",TL="_green_x2w5i_665",PL="_red_x2w5i_667",NL="_statLabel_x2w5i_671",qL="_retryBtn_x2w5i_685",ie={quizContainer:Qw,progressBar:Jw,progressDot:Ww,done:Yw,correct:Kw,wrong:Xw,active:Zw,questionCard:eL,questionNumber:tL,questionType:aL,questionText:rL,codeContext:nL,codeGap:iL,options:sL,option:lL,disabled:oL,selected:cL,optionLetter:uL,optionCode:dL,optionText:pL,feedback:mL,fadeIn:hL,feedbackTitle:xL,navRow:gL,nextBtn:fL,results:jL,scoreCircle:vL,great:bL,good:yL,low:SL,scoreNumber:EL,scoreLabel:CL,resultsTitle:AL,resultsSubtitle:wL,resultsSummary:LL,statBox:RL,statNumber:ML,green:TL,red:PL,statLabel:NL,retryBtn:qL},kL=["A","B","C","D"];function OL({questions:t,onFinish:a}){const[r,n]=w.useState(0),[i,s]=w.useState(null),[l,o]=w.useState(!1),[c,u]=w.useState([]),x=t[r],m=i===(x==null?void 0:x.correctIndex),j=r>=t.length,S=w.useCallback(E=>{l||(s(E),o(!0),u(g=>[...g,E===x.correctIndex]))},[l,x]),v=w.useCallback(()=>{s(null),o(!1),n(E=>E+1)},[]),y=w.useCallback(()=>{n(0),s(null),o(!1),u([])},[]);if(j){const E=c.filter(Boolean).length,g=t.length,h=Math.round(E/g*100),b=h>=80?"great":h>=50?"good":"low",M=h>=80?"Molt be! Domines MVC!":h>=50?"Bon treball! Repassa els errors.":"Cal repassar una mica mes. Torna-ho a intentar!";return e.jsxs("div",{className:ie.results,children:[e.jsxs("div",{className:`${ie.scoreCircle} ${ie[b]}`,children:[e.jsxs("span",{className:ie.scoreNumber,children:[h,"%"]}),e.jsx("span",{className:ie.scoreLabel,children:"encerts"})]}),e.jsx("h2",{className:ie.resultsTitle,children:M}),e.jsxs("p",{className:ie.resultsSubtitle,children:["Has encertat ",E," de ",g," preguntes"]}),e.jsxs("div",{className:ie.resultsSummary,children:[e.jsxs("div",{className:ie.statBox,children:[e.jsx("div",{className:`${ie.statNumber} ${ie.green}`,children:E}),e.jsx("div",{className:ie.statLabel,children:"Correctes"})]}),e.jsxs("div",{className:ie.statBox,children:[e.jsx("div",{className:`${ie.statNumber} ${ie.red}`,children:g-E}),e.jsx("div",{className:ie.statLabel,children:"Incorrectes"})]})]}),e.jsx("button",{className:ie.retryBtn,onClick:y,children:"Torna a intentar-ho"})]})}return e.jsxs("div",{className:ie.quizContainer,children:[e.jsx("div",{className:ie.progressBar,children:t.map((E,g)=>{let h=ie.progressDot;return g<c.length?h+=` ${c[g]?ie.correct:ie.wrong}`:g===r&&(h+=` ${ie.active}`),e.jsx("div",{className:h},g)})}),e.jsxs("div",{className:`${ie.questionCard} ${l?m?ie.correct:ie.wrong:""}`,children:[e.jsxs("div",{className:ie.questionNumber,children:["Pregunta ",r+1," de ",t.length,e.jsx("span",{className:ie.questionType,children:x.type})]}),e.jsx("div",{className:ie.questionText,children:x.question}),x.code&&e.jsx("div",{className:ie.codeContext,children:e.jsx("pre",{dangerouslySetInnerHTML:{__html:x.code}})}),e.jsx("div",{className:ie.options,children:x.options.map((E,g)=>{let h=`${ie.option}`;return l?(h+=` ${ie.disabled}`,g===x.correctIndex?h+=` ${ie.correct}`:g===i&&(h+=` ${ie.wrong}`)):g===i&&(h+=` ${ie.selected}`),e.jsxs("button",{className:h,onClick:()=>S(g),children:[e.jsx("span",{className:ie.optionLetter,children:kL[g]}),e.jsx("span",{className:E.isCode?ie.optionCode:ie.optionText,children:E.text})]},g)})}),l&&e.jsxs("div",{className:`${ie.feedback} ${m?ie.correct:ie.wrong}`,children:[e.jsx("div",{className:ie.feedbackTitle,children:m?"Correcte!":"Incorrecte!"}),e.jsx("div",{children:x.explanation})]}),l&&e.jsx("div",{className:ie.navRow,children:e.jsx("button",{className:ie.nextBtn,onClick:v,children:r<t.length-1?"Seguent pregunta":"Veure resultats"})})]})]})}const DL=[{type:"On va?",question:"A quina capa MVC pertany aquest codi?",code:`<span style="color:#ffd700">public class</span> <span style="color:#e5c07b">Alumne</span> {
